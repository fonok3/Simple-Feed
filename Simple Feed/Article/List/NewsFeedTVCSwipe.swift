...
extension NewsFeedTVC {
    override func tableView(_: UITableView, leadingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        if let article = fetchedResultsController?.object(at: indexPath) {
            let readAction = UIContextualAction(style: (article.read || article.tagged) ? .normal : .destructive, title: NSLocalizedString(article.read ? "UNREAD" : "READ", comment: "read")) { _, _, completionHandler in
    ...
    override func tableView(_: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        if let obj = fetchedResultsController?.object(at: indexPath) {
            let readAction = UIContextualAction(style: .normal, title: NSLocalizedString(obj.tagged ? "UNTAG" : "TAG", comment: "tag")) { _, _, completionHandler in
                completionHandler(true)
...
extension NewsFeedTVC {
    override func tableView(_: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
        if let obj = fetchedResultsController?.object(at: indexPath) {
            let readAction = UITableViewRowAction(style: .normal, title: NSLocalizedString(obj.read ? "UNREAD" : "READ", comment: "read")) { _, _ in

                obj.changeReadStatus()
            }
            readAction.backgroundColor = UIColor(red: 0, green: 122 / 255, blue: 1, alpha: 1)
            let tagAction = UITableViewRowAction(style: .normal, title: NSLocalizedString(obj.tagged ? "UNTAG" : "TAG", comment: "tag"), handler: { _, _ in
                obj.changeTaggingStatus()
            })
